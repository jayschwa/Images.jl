# readWAL.jl, adds an imread function for Quake .wal images
...
import Images:Image,imread
function imread{S<:IO}(stream::S, ::Type{Images.QuakeWAL})
	# name of texture
	seek(stream, 0)
	name = lowercase(rstrip(bytestring(read(stream, Uint8, 32)),"\0"))

	# size
	width = read(stream, Uint32)
	height = read(stream, Uint32)

	# mipmap byte offsets
	offset = read(stream, Int32, 4)

	# name of next texture (if animated)
	next_name = lowercase(rstrip(bytestring(read(stream, Uint8, 32)),"\0"))

	flags = read(stream, Uint32)
	contents = read(stream, Uint32)
	value = read(stream, Uint32)
